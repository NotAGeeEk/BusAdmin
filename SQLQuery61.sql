...
CREATE PROCEDURE [dbo].[uspDeleteBusDetailsByBusID]
(
    @BusID int
)
AS
BEGIN
    ...
    -- Delete the specified BusID from both BusMaster and RouteDetails tables
    DELETE FROM dbo.BusMaster WHERE BusId = @BusID;
    DELETE FROM dbo.RouteDetails WHERE BusID = @BusID;
    
    PRINT 'Bus details deleted successfully.';
END
GO